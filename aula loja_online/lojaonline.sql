...
CREATE TABLE Produtos (
  ...
  estoque INT DEFAULT 0 CHECK (estoque >= 0),  -- Restrição CHECK para garantir que o estoque seja >= 0
  categoria_id INT NOT NULL,  -- Definição correta da coluna categoria_id
  FOREIGN KEY (categoria_id) REFERENCES Categorias(categoria_id)  -- Chave estrangeira para Categorias
);
...
CREATE TABLE ItensPedido (
  ...
  quantidade INT NOT NULL CHECK (quantidade > 0),  -- Garantir que a quantidade seja maior que 0
  preco_unitario DECIMAL(10, 2) NOT NULL CHECK (preco_unitario > 0),  -- Garantir que o preço seja maior que 0
  FOREIGN KEY (pedido_id) REFERENCES Pedidos(pedido_id),  -- Chave estrangeira para a tabela Pedidos
  FOREIGN KEY (produto_id) REFERENCES Produtos(produto_id)  -- Chave estrangeira para a tabela Produtos
);
